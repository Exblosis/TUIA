...
namespace Tower_Unite_Instrument_Autoplayer.GUI
{
    public partial class GraphicalUserInterface : Form
    {
        ...
        {
            InitializeComponent();
            updater = new SharpUpdater(Assembly.GetExecutingAssembly(), this, new Uri("https://raw.githubusercontent.com/Exblosis/TUIA/master/update.xml"));
        ...
        private void MakeSong()
        {
            try
            {
                ErrorTextBox.ForeColor = System.Drawing.Color.GreenYellow;
        ...
        private void LoadPieceButton_Click(object sender, EventArgs e)
        {
            if (MusicPieceListBox.SelectedIndex.Equals(0))
            {
                try
                {
                    isLoading = true;
                    Autoplayer.ResetBreaks();
                    Autoplayer.LoadPiece(NodePieceTextBox.Text);
                    //Update everything when we are done loading
                    UpdateEverything();
                    MessageBox.Show("Loading completed");
                }
        ...
        private void MusicPieceListBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            NodePieceTextBox.Text = Properties.Resources.Senbonzakura;
        }
    }
}
